...
class CExprNodeAllocatorTlsMgr
{
public:
	static dbMutex							mutex					;
	static CExprNodeAllocatorTlsMgr*		pInstance				;
	static BOOL								s_bIsUninited			;
	static map<DWORD,dbExprNodeAllocator*>  threadExprNodeAllocator	;

	static CExprNodeAllocatorTlsMgr* GetInstance(BOOL& bIsUnited)
	{
		dbCriticalSection cs(CExprNodeAllocatorTlsMgr::mutex);
		bIsUnited = s_bIsUninited;
		return pInstance;
	}
...
//FASTDB_DLL_ENTRY
class FASTDB_DLL_ENTRY dbExprNodeAllocator : public dbL2List
{ 
  private:
  ...
  public:  
	...
	static  dbExprNodeAllocator* GetInstance()
	{
		dbCriticalSection cs(CExprNodeAllocatorTlsMgr::mutex);
		
		BOOL bIsUninited = FALSE;
		CExprNodeAllocatorTlsMgr* pInstance = CExprNodeAllocatorTlsMgr::GetInstance(bIsUninited);
		...
		if(NULL == pInstance)	
		{
			CExprNodeAllocatorTlsMgr::pInstance = new CExprNodeAllocatorTlsMgr;
		...
		dbExprNodeAllocator* p = new dbExprNodeAllocator;
		if(p)
		{
			//自己建立管理内存池。
			DWORD dwThread = GetCurrentThreadId();
			...
			CExprNodeAllocatorTlsMgr::threadExprNodeAllocator[dwThread] = p;
		}
		return p;
	}
...
class FASTDB_DLL_ENTRY dbExprNode { 
  ...
  public:
    ...
    void* operator new(size_t size EXTRA_DEBUG_NEW_PARAMS) { 
        return dbExprNodeAllocator::GetInstance()->allocate();
    }

    void operator delete(void* ptr EXTRA_DEBUG_NEW_PARAMS) { 
        dbExprNodeAllocator::GetInstance()->deallocate((dbExprNode*)ptr);
    }
};
