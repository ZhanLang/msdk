...
public:
	...
	CSa()
	{
		...
		Init_sd_to_everyone(sd,sid,acl);
	...
	*/
	BOOL Init_sd_to_everyone (SECURITY_DESCRIPTOR &sd, PSID &sid, PACL &acl)
	{
	...
	void Free_sid_acl(PSID sid, PACL acl)
	{
		if(acl)
		{
			delete []acl;
...
bool local_win_socket::open(int)
{
    ...
    for (i = RD; i <= RTT; i++) {  
        ...
        sprintf(buf, "Global\\%s.%c", Name, i + '0');
        Signal[i] = CreateEvent(&theSa.sa, false, false, WC_STRING(buf));//FASTDB_SECURITY_ATTRIBUTES
    ...
    sprintf(buf, "Global\\%s.shr", Name);
    BufHnd = CreateFileMapping(INVALID_HANDLE_VALUE,&theSa.sa , PAGE_READWRITE,
                               0, sizeof(socket_buf)*2, WC_STRING(buf));//FASTDB_SECURITY_ATTRIBUTES
    if (!BufHnd) {
        Error = GetLastError();
        for (i = RD; i <= RTT; i++) {  
            CloseHandle(Signal[i]);
