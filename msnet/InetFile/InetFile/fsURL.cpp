...
fsInternetResult fsURL::Crack(LPCTSTR pszUrl, BOOL bCheckScheme)
{
	...
	if (_tcsncicmp (pszUrl, _T("file://"), 7)) 
	{
...
_lFileUrl:
	if (m_url.nScheme == INTERNET_SCHEME_FILE)
	{
														...
														
		{
			...
			_tcsncpy_s (m_szHost,_countof(m_szHost), m_szPath + 2, iPathStart - 2); 
...
VOID fsURL::FixWinInetBug()
{
	...
	_tcsncat_s (m_szPassword,_countof(m_szPassword), m_szHost, psz - m_szHost);
...
VOID fsURL::Encode(LPCTSTR psz, LPTSTR sz)
{
	while (*psz)
	{
		...
		if (c == ':' || c == '@' || c == '%')
		{
			_stprintf_s (sz2, _countof(sz2), _T("%%%x"), (int)(BYTE)c);		
		}
		else
		{
			sz2 [0] = c; sz2 [1] = 0;
