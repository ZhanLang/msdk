...
static const unsigned int scm_auiInitP[MAX_PBLOCK_SIZE] = {
...
public:
	CBlowFish(){ memset(&m_blowfish, 0, sizeof(m_blowfish));}
	
	int BlowFishInit(unsigned char *ucKey, size_t keysize)
	{
		return _BlowFishInit(&m_blowfish, ucKey, keysize);
	}

	int Encrypt(const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode = ECB)
	{
		return _Encrypt(&m_blowfish, in, siz_i,out, siz_o, iMode);
	}

	int Decrypt(const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode = ECB)
	{
		return _Decrypt(&m_blowfish, in, siz_i, out, siz_o, iMode);
	}
...
//////////////////////////////////////////////////////////////////////////
private:
	static int _BlowFishInit(Blowfish *blowfish, unsigned char *ucKey, size_t keysize)
	{
	...
	static int _Encrypt(Blowfish *blowfish, const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)
	{
		unsigned char *byte_in = NULL;
		unsigned char *byte_out = NULL;
		int nLen = 0;
		int nRet = 0;
		...
		if (siz_o < (unsigned int)(nLen*2)) return -1;
	...
	static int _Decrypt(Blowfish *blowfish, const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)
	{
		...
		if (siz_o < (unsigned int)nLen) return -1;
	...
	static void HexStr2CharStr(unsigned char *pszHexStr, int iSize, unsigned char *pucCharStr)
	{
		int i;
		unsigned char ch;
	...
	static void CharStr2HexStr(unsigned char *pucCharStr, int iSize, unsigned char *pszHexStr)
	{
		int i;
		char szHex[3];
