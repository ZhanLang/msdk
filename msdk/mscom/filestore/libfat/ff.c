...
#include "fatlib.h"		/* Register low level disk I/O functions */
...
extern struct fat_device_operations * disk_io_ops;
...
)					/* Move to zero only writes back dirty window */
{
	...
	if (wsect != sector) {	/* Changed current window */
		...
		if (fs->wflag) {	/* Write back dirty window if needed */
			if (disk_io_ops->disk_write(disk_io_ops, fs->dev, fs->win, wsect, 1) != RES_OK)
				return FR_DISK_ERR;
			fs->wflag = 0;
			if (wsect < (fs->fatbase + fs->fsize)) {	/* In FAT area */
				...
				for (nf = fs->n_fats; nf > 1; nf--) {	/* Reflect the change to all FAT copies */
					wsect += fs->fsize;
					disk_io_ops->disk_write(disk_io_ops, fs->dev, fs->win, wsect, 1);
				}
			}
		}
#endif
		if (sector) {
			if (disk_io_ops->disk_read(disk_io_ops, fs->dev, fs->win, sector, 1) != RES_OK)
				return FR_DISK_ERR;
			fs->winsect = sector;
		}
	}
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {
			...
			disk_io_ops->disk_write(disk_io_ops, fs->dev, fs->win, fs->fsi_sector, 1);
			fs->fsi_flag = 0;
		}
		/* Make sure that no pending write process in the physical drive */
		if (disk_io_ops->disk_ioctl(disk_io_ops, fs->dev, CTRL_SYNC, (void*)0) != RES_OK)
			res = FR_DISK_ERR;
	}
...
{
	if (disk_io_ops->disk_read(disk_io_ops, fs->dev, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 3;
...
)
{
	...
	*rfs = fs = FatFs[vol];			/* Return pointer to the corresponding file system object */
	...
	if (fs->fs_type) {				/* If the logical drive has been mounted */
		stat = disk_io_ops->disk_status(disk_io_ops, fs->dev);
	...
	fs->dev = dev;
	stat = disk_io_ops->disk_initialize(disk_io_ops, fs->dev);	/* Initialize low level disk I/O layer */
	if (stat & STA_NOINIT)				/* Check if the drive is ready */
		return FR_NOT_READY;
#if _MAX_SS != 512						/* Get disk sector size if needed */
	if (disk_io_ops->disk_ioctl(disk_io_ops, fs->dev, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > _MAX_SS)
	...
	if (fmt == FS_FAT32) {
		...
		if (disk_io_ops->disk_read(disk_io_ops, fs->dev, fs->win, fs->fsi_sector, 1) == RES_OK &&
...
)
{
	...
	if (disk_io_ops->disk_status(disk_io_ops, fs->dev) & STA_NOINIT)
		return FR_NOT_READY;
...
)
{
	...
	res = chk_mounted(dev, &path, &dj.fs, (BYTE)(mode & ~FA_READ));
#else
	mode &= FA_READ;
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Read maximum contiguous sectors directly */
				...
				if (disk_io_ops->disk_read(disk_io_ops, fp->fs->dev, rbuff, sect, (BYTE)cc) != RES_OK)
			...
			if (fp->flag & FA__DIRTY) {			/* Write sector I/O buffer if needed */
				if (disk_io_ops->disk_write(disk_io_ops, fp->fs->dev, fp->buf, fp->dsect, 1) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
				fp->flag &= ~FA__DIRTY;
			}
#endif
			if (fp->dsect != sect) {			/* Fill sector buffer with file data */
				if (disk_io_ops->disk_read(disk_io_ops, fp->fs->dev, fp->buf, sect, 1) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)		/* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if (disk_io_ops->disk_write(disk_io_ops, fp->fs->dev, wbuff, sect, (BYTE)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
			...
			if (fp->dsect != sect) {				/* Fill sector buffer with file data */
				if (fp->fptr < fp->fsize &&
					disk_io_ops->disk_read(disk_io_ops, fp->fs->dev, fp->buf, sect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
)
{
	...
	res = chk_mounted(dev, &path, &dj.fs, 0);
	if (res == FR_OK) {
		INIT_BUF(dj);
			...
			if (ofs) {
				...
				if (fp->fptr % SS(fp->fs) && dsc != fp->dsect) {
					...
					if (fp->flag & FA__DIRTY) {		/* Flush dirty buffer if needed */
						if (disk_io_ops->disk_write(disk_io_ops, fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK)
							ABORT(fp->fs, FR_DISK_ERR);
						fp->flag &= ~FA__DIRTY;
					}
#endif
					if (disk_io_ops->disk_read(disk_io_ops, fp->fs->drv, fp->buf, dsc, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
#endif
					fp->dsect = dsc;
				}
...
#endif
			if (disk_io_ops->disk_read(disk_io_ops, fp->fs->dev, fp->buf, nsect, 1) != RES_OK)
				ABORT(fp->fs, FR_DISK_ERR);
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	/* Get drive number */
	res = chk_mounted(dev, &path, fatfs, 0);
	if (res == FR_OK) {
...
)
{
	...
	res = chk_mounted(dev, &path, &dj.fs, 1);
	if (res == FR_OK) {
		INIT_BUF(dj);
		res = follow_path(&dj, path);		/* Follow the file path */
...
)
{
	...
	res = chk_mounted(dev, &path_old, &djo.fs, 1);
	if (res == FR_OK) {
		djn.fs = djo.fs;
		INIT_BUF(djo);
...
)
{
	...
	/* Check parameter sfd */
	if(sfd != PR_FDISK && sfd != PR_SFD)
		return FR_MKFS_ABORTED;

	/* Get disk statics */
	stat = disk_io_ops->disk_initialize(disk_io_ops, dev);
	if (stat & STA_NOINIT) return FR_NOT_READY;
	if (stat & STA_PROTECT) return FR_WRITE_PROTECTED;
#if _MAX_SS != 512					/* Get disk sector size */
	if (disk_io_ops->disk_ioctl(disk_io_ops, dev, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > _MAX_SS)
		return FR_DISK_ERR;
#endif
	if (disk_io_ops->disk_ioctl(disk_io_ops, dev, GET_SECTOR_COUNT, &n_vol) != RES_OK || n_vol < 128)
		return FR_DISK_ERR;
	b_vol = (sfd == PR_SFD) ? 0 : 63;	/* Volume start sector */
	n_vol -= b_vol;
	...
	if (disk_io_ops->disk_ioctl(disk_io_ops, dev, GET_BLOCK_SIZE, &n) != RES_OK) return FR_DISK_ERR;
	...
	if (sfd == PR_SFD) {
		...
		if (disk_io_ops->disk_write(disk_io_ops, dev, fs->win, 0, 1) != RES_OK)
			return FR_DISK_ERR;
		md = 0xF8;
	}
	...
	if (disk_io_ops->disk_write(disk_io_ops, dev, tbl, b_vol, 1) != RES_OK)	/* Original (VBR) */
		return FR_DISK_ERR;
	if (fmt == FS_FAT32)						/* Backup (VBR+6) */
		disk_io_ops->disk_write(disk_io_ops, dev, tbl, b_vol + 6, 1);
	...
	for (i = 0; i < N_FATS; i++) {
		...
		if (disk_io_ops->disk_write(disk_io_ops, dev, tbl, b_fat++, 1) != RES_OK)
			return FR_DISK_ERR;
	...
	if (fmt == FS_FAT32) {
		...
		disk_io_ops->disk_write(disk_io_ops, dev, tbl, b_vol + 1, 1);	/* Original (VBR+1) */
		disk_io_ops->disk_write(disk_io_ops, dev, tbl, b_vol + 7, 1);	/* Backup  (VBR+7) */
	}

	return (disk_io_ops->disk_ioctl(disk_io_ops, dev, CTRL_SYNC, (void*)0) == RES_OK) ? FR_OK : FR_DISK_ERR;
}
