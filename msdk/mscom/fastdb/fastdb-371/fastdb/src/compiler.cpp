...
//dbL2List							CExprNodeAllocatorTlsMgr::threadContextList	;
dbMutex								CExprNodeAllocatorTlsMgr::mutex				;
CExprNodeAllocatorTlsMgr*			CExprNodeAllocatorTlsMgr::pInstance			;
BOOL								CExprNodeAllocatorTlsMgr::s_bIsUninited	 = FALSE;

map<DWORD,dbExprNodeAllocator*>		CExprNodeAllocatorTlsMgr::threadExprNodeAllocator;


BOOL CExprNodeAllocatorTlsMgr::ReleaseOne()
{
	dbCriticalSection cs(CExprNodeAllocatorTlsMgr::mutex);

	DWORD dwThread = GetCurrentThreadId();
	map<DWORD,dbExprNodeAllocator*>::iterator it = CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.find(dwThread);
	if(it !=  CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.end())
	{
		if(it->second)
		{
			delete it->second;	
		}
		CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.erase(it);
	}
...
BOOL CExprNodeAllocatorTlsMgr::Uninit()
{
	dbCriticalSection cs(CExprNodeAllocatorTlsMgr::mutex);
	map<DWORD,dbExprNodeAllocator*>::iterator it = CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.begin();
	for(;it != CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.end(); ++it)
	{
		if(it->second)	
			delete it->second;
	}
	CExprNodeAllocatorTlsMgr::threadExprNodeAllocator.clear();

	if(NULL != CExprNodeAllocatorTlsMgr::pInstance)
	{
		delete CExprNodeAllocatorTlsMgr::pInstance;
		CExprNodeAllocatorTlsMgr::pInstance = NULL;
	}

	CExprNodeAllocatorTlsMgr::s_bIsUninited = TRUE;
...
dbExprNodeAllocator::dbExprNodeAllocator()
{
	...
	if (node == NULL) {
		...
		//wsprintfW(tempBuffer,L"::::::  0x%x  \n",(DWORD)seg);
		//OutputDebugStringW(tempBuffer);	
...
void dbCompiler::deleteNode(dbExprNode* node) {
    dbMutex& mutex = dbExprNodeAllocator::GetInstance()->getMutex();
    dbCriticalSection cs(mutex);
    delete node;
}
