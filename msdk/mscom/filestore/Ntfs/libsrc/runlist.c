...
--*/
{
	...
	buf = (const u8*)attr + le16_to_cpu(attr->X.mapping_pairs_offset); // 定位到数据运行信息处(run)
...
// can investigate it further!
	...
	deltaxcn = sle64_to_cpu(attr->X.highest_vcn);
...
mpa_err:
	...
	if ( !attr->X.lowest_vcn ) // 起始的VCN为0,表明this is the base extent.
	{
		VCN max_cluster;
		max_cluster = ( (sle64_to_cpu(attr->X.allocated_size) + vol->cluster_size - 1) >> vol->cluster_size_bits ) - 1;
