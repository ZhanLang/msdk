...
const INT PARSE_FLAG = parse_declaration_node|parse_no_entity_translation;
...
public:
	...
	virtual HXMLTREEITEM LocalItem(HXMLTREEITEM hItem, LPCTSTR lpPathName, BOOL bCreate = FALSE)
	{
		...
		while ((nPath = (INT)sPathName.find(_T('\\'))) != -1)
		{
			...
			sPathName = sPathName.substr(nPath + 1);//&sPathName[nPath + 1];
...
// 				sPathName = _T("");
	...
	virtual HXMLTREEITEM InsertChild(HXMLTREEITEM hItem, HXMLTREEITEM hAdd)
	{
		TESTIFNULL_AS_ROOT(hItem);
		rapid_node* pNode = (rapid_node*)hItem;
		rapid_node* pChild = m_Doc.clone_node((rapid_node*)hAdd);
		pNode->append_node( pChild );
		return pChild;
	}
	...
	LPCTSTR GetText(HXMLTREEITEM hItem)
	{
		...
		case node_element:
			for(pNode = pNode->first_node();
				...
				pNode = pNode->next_sibling())
			{
				if (node_data == pNode->type() || node_cdata == pNode->type())
				{
					return pNode->value();
				}
			}
	...
	virtual LPCTSTR GetAttribute(HXMLTREEITEM hItem, LPCTSTR lpAttName = NULL ,LPCTSTR lpDef = NULL)
	{
		TESTIFNULL_AS_ROOT(hItem);
	...
	INT GetAttributeInt(HXMLTREEITEM hItem, INT nDef = 0 ,LPCTSTR lpAttName = NULL )
	{
