...
SHORT fsFtpFile::OpenSocket()
{
	...
	TCHAR ipAddr [100]={0};
	_stprintf_s (ipAddr,_countof(ipAddr), _T("%d,%d,%d,%d,%d,%d"), (int) (BYTE) he->h_addr_list [0][0], (int) (BYTE) he->h_addr_list [0][1], (int) (BYTE) he->h_addr_list [0][2], (int) (BYTE) he->h_addr_list [0][3],
...
fsInternetResult fsFtpFile::QuerySize(LPCTSTR pszFilePath)
{
	...
	_tcscpy_s (szFile,_countof(szFile), pszFilePath);
	...
	if (pszFile)
	{
		if (pszFile - pszFilePath)
		{
			...
			_stprintf_s (szCmd,_countof(szCmd) ,_T("CWD %s"), szFile);	
			...
			if (!FtpCommand (hServer, FALSE, m_dwTransferType, szCmd, NULL, NULL))
				...
				_tcscpy_s (szFile,_countof(szFile), pszFile + 1);	
	...
	if (bListOK == FALSE)
	{
		...
		_stprintf_s (szCmd,_countof(szCmd), _T("SIZE %s"), szFile);
...
{
	_stprintf_s (pszCmd, dwCount,_T("LIST %s"), pszFile);
...
fsInternetResult fsFtpFile::OpenEx(LPCTSTR pszFilePath, UINT64 uStartPos, UINT64 , UINT64 uUploadTotalSize)
{
	...
	_stprintf_s (szCmd,_countof(szCmd), _T("TYPE %c"), m_dwTransferType & FTP_TRANSFER_TYPE_ASCII ? 'A' : 'I');
	...
	else
	{
		...
		_stprintf_s (szCmd,_countof(szCmd), _T("PORT %s"), m_strPORT);
	...
	if (uUploadTotalSize == _UI64_MAX)
	{
		if (uStartPos) 
		{
			_stprintf_s (szCmd,_countof(szCmd), _T("REST %I64u"), uStartPos); 
		...
		_stprintf_s (szCmd,_countof(szCmd), _T("RETR %s"), szFile); 
	}
	else
	{
		if (uStartPos)
			_stprintf_s (szCmd,_countof(szCmd) ,_T("APPE %s"), szFile);
		else
			_stprintf_s (szCmd, _countof(szCmd) ,_T("STOR %s"), szFile);			
	}
