...
static int cli_get(int statement, int cmd, cli_oid_t value = 0)
{
    ...
    for (column_binding* cb = s->columns; cb != NULL; cb = cb->next) { 
        ...
        if (cb->set_fnc != NULL) { 
            ...
            if (cb->var_type >= cli_asciiz && (cb->var_type <= cli_array_of_string || cb->var_type == cli_array_of_wstring || cb->var_type == cli_wstring)) { 
                int len = unpack4(p);
                p += 4;
                ...
                /*因为cp没有构造函数,变量没有初始化,relese版会执行到这里
				造成每个字串长度都是33,bug了.
				if (cb->var_len != NULL && *cb->var_len > 0) { 
                    if (n > *cb->var_len) { 
                        n = *cb->var_len;
                    }
                    *cb->var_len = n;
					char tempBuffer[MAX_PATH];
					_itoa(n,tempBuffer,10);
					OutputDebugStringA("因为cp没有构造函数,变量没有初始化,relese版会执行到这里:");
					OutputDebugStringA(cb->name);
					OutputDebugStringA("cli, remote wstring len:");
                ...
                if (cb->var_type == cli_wstring || cb->var_type == cli_pwstring) { 
                    ...
                    memcpy(dst, p, nTempLen*sizeof(wchar_t));
					//解包时还要用原长度.
					...
					char tempBuffer[MAX_PATH];
					_itoa(n,tempBuffer,10);
					OutputDebugStringA("cli, remote wstring name:");
					OutputDebugStringA(cb->name);
					OutputDebugStringA("cli, remote wstring len:");
					OutputDebugStringA(tempBuffer);
					OutputDebugStringA("cli,retmote context:");				
					OutputDebugStringW((LPCTSTR)dst);
					OutputDebugStringA("\n");
#endif
