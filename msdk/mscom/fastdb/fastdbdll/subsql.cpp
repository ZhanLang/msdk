...
//用真正的宽字符.
#ifdef _WIN32 
#define mbstowcs_new(_dst, _src, _size) (MultiByteToWideChar(CP_ACP, 0, (_src), -1, (_dst), (int)(_size))-1)
#define wcstombs_new(_dst, _src, _size) (WideCharToMultiByte(CP_ACP, 0, (_src), -1, (_dst), (int)(_size), NULL, NULL)-1)
#endif
...
dbSubSql::dbSubSql(dbAccessType accessType,int iInitSize,int iIndexSize)
: dbDatabase(accessType,iInitSize,dbDefaultExtensionQuantum,iIndexSize)
{
    static struct { 
        char* name;
        int   tag;
...
int  dbSubSql::get(CDBSqlStream& stream)
{
	return stream.Get();
}
void dbSubSql::unget(int ch,CDBSqlStream& stream)
{
	stream.Unget(ch);
}
...
void dbSubSql::error(CDBSqlStream& stream,char const* msg)
{
   ...
   dbDatabaseThreadContext* ctx = threadContext.get();
    ...
    try {
		...
		OutputDebugString(L"----dbSubSql::error!\n");
        //handleError(stream,QueryError, msg, tknPos > 0 ? tknPos - 1 : 0);
    } catch(dbException) {}
#else
    dbDatabaseThreadContext* ctx = threadContext.get();
    if (ctx != NULL) { 
...
int dbSubSql::scan(CDBSqlStream& stream) 
{
	int i, ch, digits;

	if (stream.ungetToken >= 0) { 
		int tkn = stream.ungetToken;
		stream.ungetToken = -1;
		return tkn;
	}
	bool dotIsIdentifierChar = stream.dotIsPartOfIdentifier;
	stream.dotIsPartOfIdentifier = false;
nextToken:
	do { 
		if ((ch = get(stream)) == EOF) { 
			return tkn_eof;
		}
	} while (isspace(ch));

	stream.tknPos = stream.pos;
	switch (ch) { 
	  case '*':
		  return tkn_all;
	  case '(':
	  ...
	  case '\'':
		  ...
		  while (true) { 
			  ...
			  if (ch == '\'') { 
				  ...
				  unget(ch,stream);
				  error(stream,"New line within character constant");
				  return tkn_error;
			  }
			  if (i+1 == stream.buflen) { 
				  char* newbuf = new char[stream.buflen*2];
	  ...
	  case '+':
		  ...
		  do { 
			  stream.buf[i++] = ch;
			  if (i == stream.buflen) { 
				  error(stream,"Numeric constant too long");
				  return tkn_error;
			  }
			  ch = get(stream);
		  } while (ch != EOF 
		  ...
		  unget(ch,stream);
		  stream.buf[i] = '\0';
		  if (sscanf(stream.buf, INT8_FORMAT "%n", &stream.ival, &digits) != 1) { 
			  error(stream,"Bad integer constant");
			  return tkn_error;
		  }
		  if (digits != i) { 
			  if (sscanf(stream.buf, "%lf%n", &stream.fval, &digits) != 1 || digits != i) {
				  error(stream,"Bad float constant");
				  return tkn_error;
			  }
			  return tkn_fconst;
		  } 
	  ...
	  case '#':
		  ...
		  while (true) { 
			  ch = get(stream);
			  if (ch >= '0' && ch <= '9') { 
				  stream.ival = (stream.ival << 4) + ch-'0';
			  } else if (ch >= 'a' && ch <= 'f') {
				  stream.ival = (stream.ival << 4) + ch-'a'+10;
			  } else if (ch >= 'A' && ch <= 'F') {
				  stream.ival = (stream.ival << 4) + ch-'A'+10;
			  } else { 
				  unget(ch,stream);
	  ...
	  default:
		  if (isalpha(ch) || ch == '$' || ch == '_') { 
			  ...
			  do { 
				  stream.buf[i++] = ch;
				  if (i == stream.buflen) { 
					  error(stream,"Identifier too long");
					  return tkn_error;
				  }
				  ch = get(stream);
			  ...
			  unget(ch,stream);
			  stream.buf[i] = '\0';
			  stream.name = stream.buf;
			  return dbSymbolTable::add(stream.name, tkn_ident);
		  } else { 
			  error(stream,"Invalid symbol");
			  return tkn_error;
		  }
	}
}
...
bool dbSubSql::expect(char* expected, int token,CDBSqlStream& stream)
{
	int tkn = scan(stream);
	if (tkn != token) { 
		if (tkn != tkn_error) { 
...
bool dbSubSql::updateTable(bool create,CDBSqlStream& stream)
{
    int tkn;
    stream.dotIsPartOfIdentifier = true;
    if (!expect("table name", tkn_ident,stream) || !expect("(", tkn_lpar,stream)) { 
        return false;
    }
    ...
    while (tkn == tkn_comma) { 
        if (nFields+1 == maxFields) { 
            error(stream,"Too many fields");
            break;
        }
        if (!expect("field name", tkn_ident,stream)) { 
            break;
        }
        int nameLen = (int)strlen(stream.buf)+1;
        fields[nFields].name = new char[nameLen];
        strcpy(fields[nFields].name, stream.buf);
        varyingLength += nameLen + 2;
        char* refTableName;
        char* inverseRefName;
        int type = parseType(refTableName, inverseRefName,stream);
        ...
        if (type == dbField::tpArray) {
            ...
            type = parseType(refTableName, inverseRefName,stream);
            if (type == dbField::tpUnknown) { 
                break;
            }
            if (type == dbField::tpArray) { 
                error(stream,"Arrays of arrays are not supported by CLI");
                break;
            }
    ...
    if (tkn == tkn_rpar) { 
        ...
        if (oldDesc != NULL) {
            if (create) { 
                error(stream,"Table already exists");
                return false;
            }
        } else { 
            if (!create) { 
                error(stream,"Table not found");
                return false;
            }
        }
        dbTable* table; 
...
int dbSubSql::parseType(char*& refTableName, char*& inverseFieldName,CDBSqlStream& stream)
{
    switch (scan(stream)) { 
      case tkn_bool:
      ...
      case tkn_array:
        return expect("of", tkn_of,stream) ? dbField::tpArray : dbField::tpUnknown;
      case tkn_string:
        return dbField::tpString;
      case tkn_wstring:
        return dbField::tpWString;
      case tkn_reference:
        if (expect("to", tkn_to,stream) && expect("referenced table name", tkn_ident,stream)) {
            refTableName = new char[strlen(stream.buf)+1];
            strcpy(refTableName, stream.buf);
            int tkn = scan(stream);
            if (tkn == tkn_inverse) {
                if (!expect("inverse reference field name", tkn_ident,stream)) { 
                    return dbField::tpUnknown;
                }
                inverseFieldName = new char[strlen(stream.buf)+1];
                strcpy(inverseFieldName, stream.buf);
            } else { 
                inverseFieldName = NULL;
                stream.ungetToken = tkn;
            }
      ...
      default:
        error(stream,"Field type expected");
    }
    return dbField::tpUnknown;
}

int dbSubSql::readExpression(CDBSqlStream& stream)
{
    int i, ch;
    int nesting = 0;
    for (i = 0; (ch = get(stream)) != ';' && (ch != ',' || nesting != 0)  && ch != EOF; i++) { 
        if (ch == '(') { 
            nesting += 1;
        } else if (ch == ')') { 
            nesting -= 1;
...
bool dbSubSql::readCondition(CDBSqlStream& stream)
{
    int i, ch;
    for (i = 0; (ch = get(stream)) != ';' && ch !=  EOF; i++) { 
        if (i+1 >= stream.buflen) { 
            char* newbuf = new char[stream.buflen*2];
            memcpy(newbuf, stream.buf, stream.buflen);
            delete[] stream.buf;
    ...
    stream.buf[i] = '\0';
    if (ch != ';') { 
        error(stream,"unexpected end of input");
        return false;
    } 
    return true;
}
                                  ...
                                  dbFieldDescriptor* first)
{
    ...
    do { 
        ...
        if (fd->type == dbField::tpArray) { 
			...
			//	return -1;
			//}
			//计算ansi字串,转成宽串后的长度.把CHAR 扩展为WCHAR
			offs += ((int)strlen(node->sval) + 1) * sizeof(WCHAR);
		} 
            ...
            switch (fd->type) {
              ...
              case dbField::tpWString:
               /* ((dbVarying*)(dst+fd->dbsOffs))->offs = offs;
                len = mbstowcs_new(NULL, node->sval, 0);
                ((dbVarying*)(dst+fd->dbsOffs))->size = len+1;
                mbstowcs_new((wchar_t*)(dst + offs), node->sval, len);
                *((wchar_t*)(dst + offs) + len) = '\0';
                offs += len*sizeof(wchar_t);*/
				  ((dbVarying*)(dst+fd->dbsOffs))->offs = offs;
				  len = mbstowcs_new(NULL, node->sval, 0);
				  ((dbVarying*)(dst+fd->dbsOffs))->size = len+(1*sizeof(wchar_t));
				  mbstowcs_new((wchar_t*)(dst + offs), node->sval, len);
				  *((wchar_t*)(dst + offs) + len) = L'\0';
				  offs += len*sizeof(wchar_t) + 1 * sizeof(wchar_t);
                break;
            }
#ifdef AUTOINCREMENT_SUPPORT
        } else if (node->type == dbList::nAutoinc) {        
...
int dbSubSql::readValues(dbList** chain,CDBSqlStream& stream)
{
    int i, n = 0;
    int tkn;
    dbList* node;
    ...
    while (true) { 
        switch (scan(stream)) { 
          case tkn_lpar:
            ...
            i = readValues(&node->aggregate.components,stream);
            if (i < 0) { 
                return -1;
            }
          ...
          case tkn_fconst:
            ...
            node->fval = stream.fval;
            break;
          case tkn_sconst:
            node = new dbList(dbList::nString);
            node->sval = new char[strlen(stream.buf)+1];
            strcpy(node->sval, stream.buf);
            break;
          ...
          case tkn_error:
            return -1;
          default:
            error(stream,"Syntax error in insert list");
            return -1;
        }
        *chain = node;
        chain = &node->next;
        n += 1;
        if ((tkn = scan(stream)) == tkn_rpar) { 
            return n;
        }
        if (tkn != tkn_comma) {   
...
dbFieldDescriptor* dbSubSql::readFieldName(CDBSqlStream& stream,int terminator)
{
    int tkn;

    if (expect("table name", tkn_ident,stream)) { 
        dbTableDescriptor* desc;
        dbFieldDescriptor* fd;
        if ((desc = findTable(stream.name)) == NULL) { 
            error(stream,"No such table in database");
            return NULL;
        }       
        if (expect(".", tkn_dot,stream) && expect("field name", tkn_ident,stream)) { 
            if ((fd = desc->findSymbol(stream.name)) == NULL) { 
                error(stream,"No such field in the table");
                return NULL;
            } else if (fd->type == dbField::tpArray) {  
                error(stream,"Array components can not be indexed");
                return NULL;
            }
        } else { 
            return NULL;
        }
        while ((tkn = scan(stream)) != terminator) { 
            if (tkn != tkn_dot) { 
                error(stream,"'.' expected");
                return NULL;
            }   
            if (expect("field name", tkn_ident,stream)) { 
                if ((fd = fd->findSymbol(stream.name)) == NULL) { 
                    error(stream,"No such field in the table");
                    return NULL;
                } else if (fd->type == dbField::tpArray) {      
                    error(stream,"Array components can not be indexed");
        ...
        if (fd->type == dbField::tpStructure) { 
            error(stream,"Structures can not be indexed");
            return NULL;
        } 
        return fd;
    }
         ...
         switch (fd->type) {
           ...
           case dbField::tpWString:
           {
               if (expr->type == tpString) { 
                   size_t len = mbstowcs_new(NULL, (char*)sattr.base, 0);
                   wchar_t* buf = new wchar_t[len+1];
                   mbstowcs_new(buf, (char*)sattr.base, len);
                   buf[len] = '\0';
                   *(wchar_t**)dst = buf; 
...
bool dbSubSql::parse(CDBSqlStream& stream,dbAnyCursor** ppCursor,BOOL* pIsGetCount) 
{
	...
	while (true) { 
		...
		switch (tkn) { 
		  case tkn_update:
			  if (!opened) { 
				  error(stream,"Database not opened");
				  continue;
			  }
			  if (accessType == dbReadOnly || accessType == dbConcurrentRead) { 
				  error(stream,"Operation is not possible in read-only mode");
				  continue;
			  }
			  stream.dotIsPartOfIdentifier = true;
			  if (expect("table name", tkn_ident,stream)) {
				  ...
				  if (setjmp(ctx->unwind) == 0) {
					  ...
					  while (true) { 
						  ...
						  dbFieldDescriptor* fd = desc->findSymbol(stream.name);
						  if (fd == NULL) { 
							  error(stream,"No such field in the table");
							  goto updateCleanup;
						  }
						  if (fd->type > dbField::tpArray && fd->type != dbField::tpWString && fd->type != dbField::tpRectangle) { 
							  error(stream,"Field can not be updated");
							  goto updateCleanup;
						  }
						  elem->field = fd;
						  elem->next = elems;
						  elems = elem;
						  if (!expect("=", tkn_eq,stream)) { 
							  goto updateCleanup;
						  }
						  startPos = stream.pos;
						  int ch = readExpression(stream);
						  if (ch == EOF) { 
							  error(stream,"unexpected end of input");
							  goto updateCleanup;
						  } 
						  condition = strstr(stream.buf, "where");
						  if (condition != NULL) {
							  *condition = '\0';
						  }
						  dbExprNode* expr = ctx->compiler.compileExpression(desc, stream.buf, startPos);
						  if (expr == NULL) { 
							  goto updateCleanup;
						  }
						  if (expr->type > tpReference && expr->type != tpList) {
							  error(stream,"Invalid expression type");
							  goto updateCleanup;
						  }
						  elem->value = expr;
					  ...
					  if (condition != NULL) { 
						  stream.query.pos = startPos + (int)(condition - stream.buf) + 5;
						  stream.query = (char const*)(condition + 5);
						  select(&cursor, stream.query);
						  if (stream.query.compileError()) { 
							  goto updateCleanup;
						  }                                   
					  } else { 
						  select(&cursor);
...
updateCleanup:
		  ...
		  case tkn_select:
			  ...
			  outputOid = true;
			  count = false;
			  if ((tkn = scan(stream)) == tkn_all) { 
				  outputOid = false;
				  tkn = scan(stream);
			  } else if (tkn == tkn_count) { 
				  if (!expect("'('", tkn_lpar,stream)
					  || !expect("'*'", tkn_all,stream)
					  || !expect("')'", tkn_rpar,stream))
				  {
					  continue;
				  }
				  count = true;
			  ...
			  if (tkn != tkn_from) {
				  while (true) { 
					  if (tkn != tkn_ident) { 
						  error(stream,"Field name or 'from' expected");
					  }
					  dbFieldDescriptor* column = new dbFieldDescriptor(stream.name);
					  if (columns != NULL) { 
						  column->next = columns;
			  ...
			  if (tkn != tkn_from) {
				  ...
				  error(stream,"FROM expected");
				  continue;
			  }
			  stream.dotIsPartOfIdentifier = true;
			  if (scan(stream) != tkn_ident) {
				  deleteColumns(columns);
				  error(stream,"Table name expected");
				  continue;
			  }
			  if ((desc = findTable(stream.name)) != NULL) {     
				  //dbAnyCursor cursor(*desc, dbCursorViewOnly, NULL);
				  dbAnyCursor* pCursor = new dbAnyCursor(*desc, dbCursorViewOnly, NULL);
				  if(NULL == pCursor)
					  break;
				  stream.query.pos = stream.pos;
				  dbDatabaseThreadContext* ctx = threadContext.get();
				  ctx->interactive = true;
				  ctx->catched = true;
#ifdef THROW_EXCEPTION_ON_ERROR
				  ...
				  if (setjmp(ctx->unwind) == 0) {
#endif
					  if (readCondition(stream)) { 
						  stream.query = (char const*)stream.buf;
						  pCursor->reset();
						  select(pCursor, stream.query);
						  if (stream.query.compileError()) { 
							  deleteColumns(columns);
							  dbExprNodeAllocator::GetInstance()->reset();
							  ctx->catched = false;
		  ...
		  case tkn_open:
			  if (expect("database name", tkn_sconst,stream)) 
			  { 
				  char* databaseName = new char[strlen(stream.buf)+1];
				  strcpy(databaseName, stream.buf);
				  char* fileName = NULL;
				  if ((tkn = scan(stream)) == tkn_sconst) 
				  { 
					  fileName = stream.buf;
				  } else if (tkn != tkn_semi) 
				  { 
					  delete[] databaseName;
					  error(stream,"database file name expected");
					  continue;
				  }
				  if (opened)
				  { 
					  close();
					  while (stream.droppedTables != NULL) 
					  { 
						  dbTableDescriptor* next = stream.droppedTables->nextDbTable;
						  delete stream.droppedTables; 
						  stream.droppedTables = next;
					  }
					  opened = false;
				  ...
				  if (!open(databaseName,fileName, (unsigned)INFINITE, (unsigned)transactionCommitDelay)) 
				  { 
...
//						  scheduleBackup(WC_STRING(backupName), period);                     
		  ...
		  case tkn_table:
			  stream.dotIsPartOfIdentifier = true;
			  if (expect("table name", tkn_ident,stream)) { 
				  desc = findTable(stream.name);
				  if (desc == NULL) { 
					  error(stream,"No such table in database");
				  } else { 
					  dropTable(desc);
					  if (desc == stream.existedTables) { 
						  stream.existedTables = desc->nextDbTable;
					  }
					  unlinkTable(desc);
					  desc->nextDbTable = stream.droppedTables;
					  stream.droppedTables = desc;
				  }
			  }
			  break;
		  case tkn_hash:
			  fd = readFieldName(stream);
			  if (fd != NULL) { 
				  if (fd->hashTable == 0) { 
					  error(stream,"This field is not hashed");
				  } else { 
					  dropHashTable(fd);
				  }
			  }
		  ...
		  case tkn_index:
			  ...
			  if (fd != NULL) { 
				  if (fd->tTree == 0) { 
					  error(stream,"There is no index for this field");
				  } else { 
					  dropIndex(fd);
				  }
			  }
			  break;
		  default:
			  error(stream,"Expecting 'table', 'hash' or 'index' keyword");
			  continue;
			  }
			  break;
		  ...
		  case tkn_backup:
			...
			/*  if (!opened) { 
			  ...
			  compactify = false;
			  if ((tkn = scan(stream)) == tkn_compactify) {
				  compactify = true;
				  tkn = scan(stream);
			  }
			  if (tkn != tkn_sconst) { 
				  error(stream,"Backup file name expected");
			  } else { 
				  if (!backup(WC_STRING(stream.buf), compactify)) { 
		  ...
		  case tkn_create:
			...
			/*  if (monitor->users != 1) { 
			  ...
			  switch (scan(stream)) { 
		  case tkn_hash:
			  if (!expect("on", tkn_on,stream)) { 
				  continue;
			  }
			  fd = readFieldName(stream);
			  if (fd != NULL) { 
				  if (fd->hashTable != 0) { 
					  error(stream,"This field is already hashed");
				  } else { 
					  createHashTable(fd);
				  }
			  }
		  ...
		  case tkn_index:
			  ...
			  if (fd != NULL) { 
				  if (fd->tTree != 0) { 
					  error(stream,"Index already exists");
				  } else { 
					  createIndex(fd);
				  }
			  }
		  ...
		  case tkn_rename:
			  ...
			  fd = readFieldName(stream,tkn_to);
			  if (fd != NULL) { 
				  if (expect("new field name", tkn_ident,stream) && expect(";", tkn_semi,stream)) { 
					  beginTransaction(dbExclusiveLock);
					  ...
					  fd->name = stream.name;
					  delete[] fd->longName;
					  fd->longName = new char[strlen(stream.name)+1];
					  strcpy(fd->longName, stream.name);
		  ...
		  case tkn_alter:
			...
			/*  if (monitor->users != 1) { 
			  ...
			  switch (scan(stream)) {
		  case tkn_table:
			  updateTable(false,stream);
			  break;
		  default:
			  error(stream,"Expecting 'table' keyword");
			  continue;
			  }
			  break;
		  ...
		  case tkn_insert:
			  ...
			  if (expect("into", tkn_into,stream)) {
				  stream.dotIsPartOfIdentifier = true;
				  if (expect("table name", tkn_ident,stream)) {
					  if ((desc = findTable(stream.name)) == NULL) { 
						  error(stream,"No such table in database");
					  ...
					  if (!expect("values", tkn_values,stream)) { 
						  continue;
					  }
					  beginTransaction(dbExclusiveLock);
					  modified = true;
					  while (expect("(", tkn_lpar,stream)) { 
						  dbList* list = NULL;
						  int n = readValues(&list,stream);
						  if (n <= 0 || !insertRecord(list, desc)) { 
							  if (n == 0) { 
								  error(stream,"Empty fields list");
							  }
		  ...
		  case tkn_delete:
			  ...
			  if (expect("FROM", tkn_from,stream)) { 
				  stream.dotIsPartOfIdentifier = true;
				  if (expect("table name", tkn_ident,stream)) {
					  if ((desc = findTable(stream.name)) == NULL) {
						  error(stream,"No such table in database");
		  ...
		  case tkn_rollback:
			  if (!opened) { 
				  ...
				  while (stream.droppedTables != NULL) { 
					  dbTableDescriptor* next = stream.droppedTables->nextDbTable;
					  linkTable(stream.droppedTables, stream.droppedTables->tableId); 
					  stream.droppedTables = next;
				  }
				  rollback();
				  while (tables != stream.existedTables) { 
		  ...
		  case tkn_export:
			  ...
			  //if (!opened) {
				 ...
				 // continue;
			  //}   
			  //if (expect("xml file name", tkn_sconst,stream)) { 
				 // if (!exportDatabaseToXml(WC_STRING(stream.buf))) { 
					//  error(stream,"Failed to open output file");
				 // }
			  //}
#endif
			  break;
		  ...
		  case tkn_import:
			  ...
			  //}
			  //if (expect("xml file name", tkn_sconst,stream)) { 
				 // if (!importDatabaseFromXml(WC_STRING(stream.buf))) { 
					//  error(stream,"Import from XML file failed");
				 // }
			  //}
#endif
			  break;
		  ...
		  case tkn_off:
			  ...
			  break;
		  default:
			  error(stream,"ON or OFF expected");
			  }
			  continue;
		  ...
		  case tkn_start:
			  if (!opened) { 
				  ...
				  tkn = scan(stream);
				  if (tkn == tkn_http) { 
					  if (expect("server", tkn_server,stream)
						  && expect("HTTP server URL", tkn_sconst,stream))
					  {
#if !SERVER_SUPPORTED
						  error(stream,"Database was build without server support");
#else
						  startHttpServer(stream.buf);
#endif
					  }
				  } else if (tkn == tkn_server && expect("server URL", tkn_sconst,stream)) { 
#if !SERVER_SUPPORTED
					  error("Database was build without server support");
#else
					  dbServer* server = dbServer::find(stream.buf);
					  if (server == NULL) { 
						  char* serverURL = new char[strlen(stream.buf)+1];
						  strcpy(serverURL, stream.buf);
						  if (expect("number of threads", tkn_iconst,stream)) { 
							  server = new dbServer(this, serverURL, (int)stream.ival);
		  ...
		  case tkn_stop:
			  tkn = scan(stream);
			  if (tkn == tkn_http) { 
				  if (expect("server", tkn_server,stream) && expect("HTTP server URL", tkn_sconst,stream))
				  {
#if !SERVER_SUPPORTED
					  error(stream,"Database was build without server support");
#else
					  stopHttpServer(stream.buf);
#endif
				  }
			  } else if (tkn == tkn_server) { 
				  if (expect("server URL", tkn_sconst,stream))
				  {
#if !SERVER_SUPPORTED
					  error("Database was build without server support");
#else
				  ...
				  error(stream,"Token 'server' expected");
			  }
			  continue;
		  case tkn_semi:
			  putchar('\n');
		...
		if (autocommit ) {	//|| !modified,注意,修改了数据时,不进行自动提交.所以要自己显式.commit.
			while (stream.droppedTables != NULL) { 
				dbTableDescriptor* next = stream.droppedTables->nextDbTable;
				delete stream.droppedTables; 
				stream.droppedTables = next;
...
void dbSubSql::handleError(CDBSqlStream& stream,dbErrorClass error, char const* msg, int arg)
{
	OutputDebugString(L"dbSubSql::handleError : error !!\n");
...
UINT thread_proc dbSubSql::httpServerThreadProc(void* arg) 
{
...
int subsql_main()
{
	...
	char* argv[] = {0};
#else // VXWORKS
int main_subsql(int argc, char* argv[]) 
{
#endif // VXWORKS
#endif // !_WINCE
