...
class FASTDB_DLL_ENTRY CQueryElementAllocatorTlsMgr
{
public:
	
	dbThreadContext<dbQueryElementAllocator> ThreadContext; 
	static dbMutex         mutex;
	static dbL2List                  threadContextList;
	
	static  CQueryElementAllocatorTlsMgr* pInstance;
};

//FASTDB_DLL_ENTRY
class FASTDB_DLL_ENTRY dbQueryElementAllocator : public dbL2List{ 
  ...
  public:
	...
	static dbQueryElementAllocator* GetInstance()
	{
		dbCriticalSection cs(CQueryElementAllocatorTlsMgr::mutex);
		if(NULL == CQueryElementAllocatorTlsMgr::pInstance )
		{
			CQueryElementAllocatorTlsMgr::pInstance = new CQueryElementAllocatorTlsMgr;
			if(NULL == CQueryElementAllocatorTlsMgr::pInstance )
				return NULL;
			
		}

		dbQueryElementAllocator* p = CQueryElementAllocatorTlsMgr::pInstance->ThreadContext.get();
		dbQueryElementAllocator* pTemp = p;
		if(NULL == p)
		{
			p = new dbQueryElementAllocator;
			if(p)
			{
				pTemp = p;
				CQueryElementAllocatorTlsMgr::threadContextList.link(p);
				CQueryElementAllocatorTlsMgr::pInstance->ThreadContext.set(p);	
				//p = CQueryElementAllocatorTlsMgr::pInstance->ThreadContext.get();
			}
