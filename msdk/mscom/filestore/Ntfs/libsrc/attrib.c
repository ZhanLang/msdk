...
--*/
{
	...
	if (!count) { return 0; } // 参数合法性检测

	// 若要读的数据超过了该属性的实际大小,那么会进行截断处理
	if ( pos + count > na->data_size ) 
	{
		if ( pos >= na->data_size ) { return 0; } // 参数合法性检测
		count = na->data_size - pos;
	}

	//
	// 若是常驻属性,从该MFT中获取Value
	//
	if ( !NAttrNonResident( na ) ) 
	{
		...
		// 在该MFT中查找对应的属性,保存到结构体ctx中. return 0表示成功
...
res_err_out:
		...
		// 关键一句,将该属性内容偏移@pos后的@count字节的数据拷贝到@b指针指向的Buffer中
...
Author: sudami [sudami@163.com]
Time  : 2009/03/01 [1:3:2009 - 15:02]
...
--*/
{
	...
	for ( ;;	a = (ATTR_RECORD*)((char*)a + le32_to_cpu(a->length)) ) 
	{
		if ( p2n(a) < p2n(ctx->mrec) 
			|| (char*)a > (char*)ctx->mrec + le32_to_cpu(ctx->mrec->bytes_allocated)
		   )
		{ 
			// 检查该属性的合法性
			break; 
		...
		if (name == AT_UNNAMED) {
			...
			&& !ntfs_names_are_equal( name, name_len, (ntfschar*)((char*)a + le16_to_cpu(a->name_offset)),a->name_length, ic, upcase, upcase_len )
			)
		{
			// 找到这个属性了
			register int rc;
		...
		ntfschar *name, u8 name_len, u8 *val, s64 size)
{
	...
	if (is_resident)
		...
		attr_rec_size = offsetof(ATTR_RECORD, X.non_resident_end) +
...
--*/
{
	...
	ntfs_log_trace("Sizes: old=%u alloc=%u attr=%u new=%u\n", (unsigned)old_size, (unsigned)alloc_size, (unsigned)attr_size, (unsigned)new_size);
...
--*/
{
	...
	if ( ntfs_attr_record_resize(m, a, (le16_to_cpu(a->X.value_offset) + new_size + 7) & ~7 ) < 0 ) { return -1; }
...
--*/
{
	...
	ntfs_log_trace("Inode 0x%llx attr 0x%x new size %lld\n", (unsigned long long)na->ni->mft_no, na->type, (long long)newsize);
...
retry:
	...
	while (!ntfs_attr_lookup(na->type, na->name, na->name_len,
		...
		if (!a->X.lowest_vcn) {
			...
			if (sparse) {
				...
				a->X.compressed_size = cpu_to_sle64(
						new_compr_size);
			}
			/*
	...
	if (finished_build) {
		...
		while (!ntfs_attr_lookup(na->type, na->name, na->name_len,
				CASE_SENSITIVE, 0, NULL, 0, ctx)) {
			if (sle64_to_cpu(ctx->attr->X.highest_vcn) !=
