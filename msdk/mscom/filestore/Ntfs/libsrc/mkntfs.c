...
/**
 * format ntfs disk liufeng 2010-7-21.
 */
#include <Windows.h>
#include <stdlib.h>
#include <stdio.h>
 ...
 * mkntfs_init_options
 */
 void mkntfs_init_options(struct mkntfs_options *opts2)
{
	if (!opts2)
		return;
		...
		const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
	...
	for (;;	a = (ATTR_RECORD*)((char*)a + le32_to_cpu(a->length))) {
		...
		/* @val is present; compare values. */
			...
			rc = memcmp(val, (char*)a +le16_to_cpu(a->X.value_offset),
					min(val_len,
					le32_to_cpu(a->X.value_length)));
			/*
		...
		const runlist *rl, const u8 *val, const s64 val_len)
{
	...
	*/
	...
	m->next_attr_instance = cpu_to_le16((le16_to_cpu(m->next_attr_instance)
			+ 1) & 0xffff);
	a->X.lowest_vcn = 0;
	a->X.highest_vcn = cpu_to_sle64(highest_vcn - 1LL);
	a->X.mapping_pairs_offset = cpu_to_le16(hdr_size + ((name_len + 7) & ~7));
	memset(a->X.reserved1, 0, sizeof(a->X.reserved1));
	/* FIXME: Allocated size depends on compression. */
	a->X.allocated_size = cpu_to_sle64(highest_vcn * g_vol->cluster_size);
	a->X.data_size = cpu_to_sle64(val_len);
	if (name_len)
	...
	if (flags & ATTR_COMPRESSION_MASK) {
		...
		a->X.compression_unit = 4;
		inited_size = val_len;
		/* FIXME: Set the compressed size. */
		a->X.compressed_size = 0;
	...
	a->X.initialized_size = cpu_to_sle64(inited_size);
	if (err < 0 || bw != val_len) {
		/* FIXME: Handle error. */
		/* deallocate clusters */
		/* remove attribute */
		...
		const u8 *val, const s64 val_len)
{
	...
	*/
	...
	for (i = 0; rl[i].length; i++)
		;
	a->X.highest_vcn = cpu_to_sle64(rl[i].vcn - 1);
	a->X.mapping_pairs_offset = cpu_to_le16(hdr_size + ((name_len + 7) & ~7));
	memset(a->X.reserved1, 0, sizeof(a->X.reserved1));
	/* FIXME: Allocated size depends on compression. */
	a->X.allocated_size = cpu_to_sle64((val_len + (g_vol->cluster_size - 1)) &
			~(g_vol->cluster_size - 1));
	a->X.data_size = cpu_to_sle64(val_len);
	a->X.initialized_size = cpu_to_sle64(val_len);
	if (name_len)
		memcpy((char*)a + hdr_size, uname, name_len << 1);
	if (flags & ATTR_COMPRESSION_MASK) {
		if (flags & ATTR_COMPRESSION_MASK & ~ATTR_IS_COMPRESSED) {
		...
		const u8 *val, const u32 val_len)
{
	...
	*/
	...
	m->next_attr_instance = cpu_to_le16((le16_to_cpu(m->next_attr_instance)
			+ 1) & 0xffff);
	a->X.value_length = cpu_to_le32(val_len);
	a->X.value_offset = cpu_to_le16(24 + ((name_len + 7) & ~7));
	a->X.resident_flags = res_flags;
	a->X.reservedR = 0;
	if (name_len)
		memcpy((char*)a + 24, uname, name_len << 1);
	if (val_len)
		memcpy((char*)a + le16_to_cpu(a->X.value_offset), val, val_len);
err_out:
	if (ctx)
		ntfs_attr_put_search_ctx(ctx);
	ntfs_ucsfree(uname);
 ...
 * Return 0 on success or -errno on error.
 */
static int add_attr_object_id(MFT_RECORD *m, const x_GUID *object_id)
{
	OBJECT_ID_ATTR oi;
	int err;

	oi.object_id= *object_id;
		...
		INDEX_ALLOCATION **idx)
{
	...
	r = (INDEX_ROOT*)((char*)a + le16_to_cpu(a->X.value_offset));
	re_end = (char*)r + le32_to_cpu(a->X.value_length);
		...
		MFT_RECORD *m, ntfschar *name, u32 name_size, ATTR_TYPES type)
{
	...
	collation_rule = ((INDEX_ROOT*)((u8*)a +
			le16_to_cpu(a->X.value_offset)))->collation_rule;
	idx_header = (INDEX_HEADER*)((u8*)a + le16_to_cpu(a->X.value_offset)
			+ 0x10);
...
do_next:
	...
	a->X.value_length = cpu_to_le32(le32_to_cpu(a->X.value_length) + idx_size);
 ...
 * Return 0 on success or -errno on error.
 */
static int index_obj_id_insert(MFT_RECORD *m, const x_GUID *guid,
		const MFT_REF ref)
{
	INDEX_ENTRY *idx_entry_new;
		...
		ntfschar* name, u32 name_len)
{
	...
	if (sle64_to_cpu(a->X.initialized_size) < i) {
		ntfs_attr_put_search_ctx(ctx);
		free(rl_index);
...
int mkntfs_redirect(struct mkntfs_options *opts2)
{
	...
	int i, err;
#if 0
	ntfs_device_default_io_ops.open			= ntfs_device_win32_open;
	ntfs_device_default_io_ops.close		= ntfs_device_win32_close;
	ntfs_device_default_io_ops.seek			= ntfs_device_win32_seek;
	ntfs_device_default_io_ops.read			= ntfs_device_win32_read;
	ntfs_device_default_io_ops.write		= ntfs_device_win32_write;
	ntfs_device_default_io_ops.pread		= ntfs_device_win32_pread;
	ntfs_device_default_io_ops.pwrite		= ntfs_device_win32_pwrite;
	ntfs_device_default_io_ops.sync			= ntfs_device_win32_sync;
	ntfs_device_default_io_ops.ioctl		= ntfs_device_win32_ioctl;
	ntfs_device_default_io_ops.stat			= ntfs_device_win32_stat;
#endif
	if (!opts2) {
	...
	if (a->non_resident) {
		...
		memcpy((char*)a + le16_to_cpu(a->X.value_offset), g_lcn_bitmap, le32_to_cpu(a->X.value_length));
	}
