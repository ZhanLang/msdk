...
fsInternetResult fsHttpFile::Open_imp(LPCTSTR pszFilePath, UINT64 uStartPos, int cTryings)
{
	...
	if (uStartPos)
		_stprintf_s (szHdr,_countof(szHdr), _T("Range: bytes=%I64u-\r\n"), uStartPos); 

	if (m_pszCookies)
		_stprintf_s (szHdr ,_countof(szHdr)+ lstrlen (szHdr), _T("Cookie: %s\r\n"), m_pszCookies); 

	if (m_pszPostData)
		_stprintf_s (szHdr,_countof(szHdr), _T("Content-Type: application/x-www-form-urlencoded\r\n"));
	...
	if (cTryings == 0)
	{
		...
		TCHAR szReq [90000] = {0};
		_stprintf_s (szReq, _countof(szReq),_T("%s %s %s\r\nReferer: %s"), pszVerb, 
...
fsInternetResult fsHttpFile::OpenEx(LPCTSTR pszFilePath, UINT64 uStartPos, UINT64 uUploadPartSize, UINT64 uUploadTotalSize)
{
	...
	if (uStartPos || uUploadPartSize != uUploadTotalSize)
	{
		...
		_stprintf_s (szHdr + lstrlen (szHdr),_countof(szHdr), _T("Range: bytes=%I64u-%I64u/%I64u"), uStartPos, 
	...
	if (m_pszCookies)
	{
		if (*szHdr)
			lstrcat (szHdr, _T("\r\n"));
		_stprintf_s (szHdr + lstrlen (szHdr),_countof(szHdr), _T("Cookie: %s"), m_pszCookies); 
	}
