...
public:
	typedef CApiMapNode<KEY,TYPE> Node;
...
template<class KEY, class TYPE> 
class CApiMapIterator								//为了支持对map进行按升序或降序遍历
{
public:
	typedef CApiMapNode<KEY,TYPE> Node;
	Node * pRBT_Node;
	CApiMapIterator(){pRBT_Node=NULL;};
	CApiMapIterator(Node* pNode){pRBT_Node=pNode;};
	void Increment();
	void Decrement();
...
template<class KEY, class TYPE, class _CMP = CApiMapNodLess<KEY>> 
class CApiMap
{
public:
	typedef CApiMapIterator<KEY,TYPE> Iterator;
	typedef CApiMapNode<KEY,TYPE>*     ValueType;
	typedef CApiMapNode<KEY,TYPE> Node;
	inline UINT Count();
	
	CApiMap();
	~CApiMap();
	CApiMap(const CApiMap &v);
	VOID  Clear();
	...
	CApiMap& operator= (const CApiMap& t);
private:
...
/*使用方式
CApiMap<CApiString, CApiString> strMap;
strMap.Insert(_T("5"), _T("hello"));
strMap.Insert(_T("4"), _T("mpimap"));

//遍历
CApiMap<CApiString,CApiString>::Iterator it = strMap.Min();
for (; it ; it++)
{
}

//查找
CApiMap<CApiString,CApiString>::ValueType value = strMap.Find(_T("2"));
*/
/*_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-*/
template<class KEY, class TYPE> 
void CApiMapIterator<KEY,TYPE>::Increment()			
{
	if(pRBT_Node==NULL)
		return;
...
template<class KEY, class TYPE> 
void CApiMapIterator<KEY,TYPE>::Decrement()
{
	if(pRBT_Node==NULL)
		return;
...
CApiMapNode<KEY,TYPE>*  CApiMap<KEY,TYPE,_CMP>::Min() const
{
	Node* pResult=m_Head;
	if(!pResult->IsSnil)
	{
...
template<class KEY, class TYPE, class _CMP> 
CApiMapNode<KEY,TYPE>*  CApiMap<KEY,TYPE,_CMP>::Max() const 
{
	Node* pResult=m_Head;
	if(!pResult->IsSnil)
	{
...
template<class KEY, class TYPE, class _CMP> 
CApiMapNode<KEY,TYPE>*  CApiMap<KEY,TYPE,_CMP>::Find(const KEY& Key)
{
	Node* pResult=m_Head;
...
template<class KEY, class TYPE, class _CMP> 
BOOL  CApiMap<KEY,TYPE,_CMP>::Insert(const KEY& Key, const TYPE& Value)
{
...
template<class KEY, class TYPE, class _CMP> 
CApiMap<KEY,TYPE,_CMP>::CApiMap(const CApiMap<KEY,TYPE,_CMP>& t)
{
	m_Snil.IsSnil		=TRUE;
	m_Snil.LelfChild	=NULL;
	m_Snil.RightChild	=NULL;
...
template<class KEY, class TYPE, class _CMP> 
BOOL CApiMap<KEY,TYPE,_CMP>::Remove(const KEY& Key)
{
...
template<class KEY, class TYPE, class _CMP> 
VOID CApiMap<KEY,TYPE,_CMP>::DeleteTree(Node *TreeNode)
{
	if(TreeNode->IsSnil)
		return ;
...
template<class KEY, class TYPE, class _CMP> 
VOID CApiMap<KEY,TYPE,_CMP>::LeftRotate(Node * TreeNode)
{
	Node * Rotate_Right;
...
template<class KEY, class TYPE, class _CMP> 
VOID CApiMap<KEY,TYPE,_CMP>::RightRotate(Node * TreeNode)
{
	Node * Rotate_Left;
...
template<class KEY, class TYPE, class _CMP> 
VOID CApiMap<KEY,TYPE,_CMP>::DeleteFixUp(Node * TreeNode)
{
	Node * Brother;
...
template<class KEY, class TYPE, class _CMP> 
VOID CApiMap<KEY,TYPE,_CMP>::Clear()
{
	DeleteTree(m_Head);
	m_Head=&m_Snil;
	m_Count=0;
...
template<class KEY, class TYPE, class _CMP> 
inline UINT CApiMap<KEY,TYPE,_CMP>::Count()
{
	return m_Count;
}

template<class KEY, class TYPE, class _CMP> 
CApiMap<KEY,TYPE,_CMP>& CApiMap<KEY,TYPE,_CMP>::operator= (const CApiMap<KEY,TYPE,_CMP>& t)
{
	Clear();
	CApiMap<KEY,TYPE,_CMP>::Iterator it = t.Min();
	for (; it; it++)
	{
