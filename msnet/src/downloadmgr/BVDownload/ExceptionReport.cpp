...
CExceptionReport::CExceptionReport()
{
	...
	_tcscat_s(FullPath, _countof(FullPath),time.c_str());


	_tcscpy_s(m_pDmpFileName,_countof(m_pDmpFileName), FullPath);
	_tcscat_s(m_pDmpFileName,_countof(m_pDmpFileName), _T(".dmp"));
...
LONG WINAPI CExceptionReport::UnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	{
		TCHAR tmp[MAX_PATH];
		_tcscpy_s(tmp,_countof(tmp), m_pDmpFileName);
...
const MSExecptionTString CExceptionReport::FormatCurrentTimeString()
{
	...
	_stprintf(szDateTime , _T("%04d-%02d-%02d(%02d-%02d-%02d)"),
