...
--*/
{
	...
	{
		ntfs_log_perror("Index root attribute missing in directory inode 0x%llx", (unsigned long long)dir_ni->mft_no);
		goto put_err_out;
	}
...
descend_into_child_node: // 递归 (进入到子节点中)
	...
	{
		if (br != -1) { errno = EIO; }
		ntfs_log_perror( "Failed to read vcn 0x%llx", (unsigned long long)vcn );
		goto close_err_out;
	}
	...
	{
		ntfs_log_debug(
			"Actual VCN (0x%llx) of index buffer is different from expected VCN (0x%llx).\n",
...
static int ntfs_filldir(ntfs_inode *dir_ni, s64 *pos, u8 ivcn_bits,
		const INDEX_TYPE index_type, void* iuTmp, INDEX_ENTRY *ie,
...
static MFT_REF ntfs_mft_get_parent_ref(ntfs_inode *ni)
{
	...
	if ((u8*)fn +	le32_to_cpu(ctx->attr->X.value_length) >
...
search:
	...
	if (ntfs_index_rm(ictx)) { goto err_out; }
