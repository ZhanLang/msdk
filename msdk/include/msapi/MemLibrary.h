...
{
	virtual ~IMemLibrary(){}
	virtual BOOL LoadLibrary(const void *, size_t) = 0;
	virtual FARPROC GetProcAddress(LPCSTR) = 0;
	virtual VOID FreeLibrary( ) = 0;
};
/*
static IMemLibrary* NewMemLibrary()
{
	return new CMemLibrary();
}
*/
//////////////////////////////////////////////////////////////////////////
//实现部分
...
class CMemLibrary : public IMemLibrary
{
public:
	CMemLibrary()
	{
	...
	virtual BOOL LoadLibrary(const void * buf, size_t size)
	{
		m_hModule = MemoryLoadLibrary(buf, size);
		if ( m_hModule )
		{
			return TRUE;
		}
	...
	virtual FARPROC GetProcAddress(LPCSTR func)
	{
		if ( m_hModule )
		{
			return MemoryGetProcAddress(m_hModule, func);
		}
...
private://
	...
	static LPVOID MemoryDefaultAlloc(LPVOID address, SIZE_T size, DWORD allocationType, DWORD protect, void* userdata)
	{
		UNREFERENCED_PARAMETER(userdata);
		return VirtualAlloc(address, size, allocationType, protect);
	}

	static BOOL MemoryDefaultFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType, void* userdata)
	{
		UNREFERENCED_PARAMETER(userdata);
	...
	static HMEMORYMODULE MemoryLoadLibrary(const void *data, size_t size)
	{
	...
	static HMEMORYMODULE MemoryLoadLibraryEx(const void *data, size_t size,
...
error:
	...
	static int MemoryCallEntryPoint(HMEMORYMODULE mod)
	{
		PMEMORYMODULE module = (PMEMORYMODULE)mod;
...
#undef MAX_LOCAL_KEY_LENGTH
	...
	static int MemoryLoadString(HMEMORYMODULE module, UINT id, LPTSTR buffer, int maxsize)
	{
		return MemoryLoadStringEx(module, id, buffer, maxsize, DEFAULT_LANGUAGE);
	}

	static int MemoryLoadStringEx(HMEMORYMODULE module, UINT id, LPTSTR buffer, int maxsize, WORD language)
	{
