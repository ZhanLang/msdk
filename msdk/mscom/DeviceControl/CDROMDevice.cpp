...
CCDROMDevice::CCDROMDevice(void)
{
}
...
BOOL CCDROMDevice::Scan(DeviceScanNotify* pNotify)
{
	RASSERT(pNotify, FALSE);
	SP_DEVINFO_DATA hDeviceInfoData = {0};
	HDEVINFO hDevInfo = SetupDiGetClassDevs((LPGUID)&GUID_DEVCLASS_CDROM, 0, 0, DIGCF_PRESENT);
	...
	{
		CCDROMDevice* pDiskDevice = new CCDROMDevice();
