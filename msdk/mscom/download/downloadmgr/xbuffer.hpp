...
public:
	XBuffer(const XBuffer& buf)
	{
		init();
		CloneFrom(buf);
	}

	XBuffer(unsigned char* lpBuf,unsigned long dwSize)
	{
		init();
		SetBuffer(lpBuf,dwSize);
	}

	XBuffer& operator =(const XBuffer& buf)
	{
		CloneFrom(buf);
		return *this;
	}
...
public:
	...
	//首先清空自己，然后进行拷贝工作
	bool CloneFrom(const XBuffer& buf)
	{
		Clear(FALSE);
