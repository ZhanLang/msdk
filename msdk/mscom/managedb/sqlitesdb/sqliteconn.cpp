...
HRESULT CSqliteConn::Connect( LPCTSTR szConn, LONG lOptions )
{
	...
	if (!parseCmd.ParseCmd(szConn))
	{
		GrpError(MODULE_NAME,DEFAULT_LEV, _T("CSqliteConn Connect(szConn=%s) E_INVALIDARG [eg:Database=c:\\db ; Pwd=maguojun]"),szConn);
		return E_INVALIDARG;;
	}
	

	TCHAR szDllPath[MAX_PATH] = {0};
	mspath::CPath::GetModulePath(g_hinstance, szDllPath, MAX_PATH);

	BOOL bUseSafe = FALSE;
	LPCTSTR lpszPwd = parseCmd.GetArg(_T("Pwd"));
	if (!(lpszPwd && _tcslen(lpszPwd)))
	{
		GrpError(MODULE_NAME, MsgLevel_Error, _T("Pwd is null [eg:Database=c:\\db ; Pwd=maguojun]"));
		return E_FAIL;
	}
	...
	LPCTSTR lpszDataBase = parseCmd.GetArg(_T("Database"));
	if (!(lpszDataBase && _tcslen(lpszDataBase)))
	{
		GrpError(MODULE_NAME, MsgLevel_Error, _T("Database is null [eg:Database=c:\\db ; Pwd=maguojun]"));
		return E_FAIL;
	}
	

	mspath::CPath::CreateDirectoryEx(lpszDataBase);
#ifdef _UNICODE
	nRet = sqlite3_open16( lpszDataBase, &m_pConn );
#else
...
#ifdef _UNICODE
	USES_CONVERSION;
	LPCSTR szpwd = W2A(lpszPwd);
#else
	LPCSTR szpwd = lpszPwd;
#endif // _UNICODE
		
	nRet = sqlite3_key(m_pConn, szpwd, strlen(szpwd));
	if (nRet != SQLITE_OK)
	{
		GrpError( MODULE_NAME, DEFAULT_LEV, _T(" sqlite3_key Error: %d\n"), nRet );
		return E_FAIL;
	}
...
HRESULT CSqliteConn::ExecuteDML( LPCTSTR szSQL, LONG* pAffected )
{
	...
	{
		GrpWarning( MODULE_NAME, 1, _T("CSqliteConn::ExecuteDML failed. sql=%s, error=%d, %s\n"), 
...
sqlite3_stmt* CSqliteConn::Compile( LPCTSTR szSQL )
{
	...
	{
		GrpWarning( MODULE_NAME, 1, _T("CSqliteConn::Compile failed. sql=%s, error=%d, %s\n"), 
			szSQL, nRet, sqlite3_errmsg16(m_pConn) );
		return NULL;
	}
}
...
HRESULT CSqliteConn::BeginTrans()
{
	...
	{
		GrpWarning( MODULE_NAME, 1, _T("CSqliteConn::BeginTrans failed. error=%d, %s\n"), nRet, sqlite3_errmsg16(m_pConn) );
		sqlite3_finalize( pStmt );
		return E_FAIL;
	}
...
HRESULT CSqliteConn::CommitTrans()
{
	...
	{
		GrpWarning( MODULE_NAME, 1, _T("CSqliteConn::CommitTrans failed. error=%d, %s\n"), nRet, sqlite3_errmsg16(m_pConn) );
		sqlite3_finalize( pStmt );
		return E_FAIL;
	}
...
HRESULT CSqliteConn::RollbackTrans()
{
	...
	{
		GrpWarning( MODULE_NAME, 1, _T("CSqliteConn::RollbackTrans failed. error=%d, %s\n"), nRet, sqlite3_errmsg16(m_pConn) );
		sqlite3_finalize( pStmt );
		return E_FAIL;
	}
